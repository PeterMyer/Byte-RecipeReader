...
export function RecipeForm() {
  ...
  const [recipeImg, setRecipeImg] = useState(
    useCase === 'edit'
      ? state.recipeData.recipeImg
        ? state.recipeData.recipeImg
        : null
      : state
      ? state.recipeImg
        ? Object.values(state.recipeImg)[0]
        : null
      : null
  );
  const [imgPreview, setImgPreview] = useState(
    useCase === 'edit'
      ? state.recipeData.recipeImg
        ? state.recipeData.recipeImg.filepath
        : { ByteIcon }
      : state
      ? state.recipeImg
        ...
        useCase === 'edit'
          ? ingredients
            ? ingredients.map((ingredient) => {
                return {
                  value: ingredient.recipeIngredient.text,
                  id: ingredient.id,
                };
              })
            : [{ value: '' }]
  ...
  const onSubmit = async (data) => {
    ...
    if (useCase === 'edit') {
      recipePayload = await createEditRecipePayload(
        data,
        userId,
        state.recipeData.ingredients,
        deleted,
